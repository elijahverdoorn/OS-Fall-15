...
int main(int argc, char *argv[])
{
  ...
  printf("Enter commands using the following syntax:  (count < %d)\n",
	 MAXSTRING);
  printf("    read <fd> <count>\n    write <fd> <string>\n    dup <fd>\n    lseek <fd> <position> <SEEK_CUR SEEK_SET SEEK_END>\n    quit\n");
  while (1) {
    ...
    printf("read write dup lseek quit:  ");
    ...
    /***** dup command *****/
      ...
      scanf("%d", &fd);
      ret_val = dup(fd);
      printf("Ran dup(), it returned %d\n", ret_val);

    /***** lseek command *****/
    } else if (!strcmp(cmd, "lseek")) {
      int position;
      char type;
      scanf("%d %d %s", &fd, &position, &type);
      if (!strcmp(type, "SEEK_CUR"))
          ret_val = lseek(fd, position, 0);
      else if (!strcmp(type, "SEEK_SET"))
	ret_val = lseek(fd, position, 1);
      else if(!strcmp(type, "SEEK_END")
	      ret_val = lseek(fd, positiion, 2);
      printf("Ran lseek(), it returned %d\n", ret_val);
